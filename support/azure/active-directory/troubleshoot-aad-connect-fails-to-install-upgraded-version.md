...
> [!WARNING]
> You might find some online documentation that includes steps to directly edit the Windows registry. However, doing this this can cause serious problems if you modify the registry incorrectly. The Microsoft Azure Active Directory Connect product team **doesn't support** editing the Windows registry.
...
Azure AD Connect Setup wizard can't detect that an Azure AD Sync Engine is installed. However, Setup fails and returns the following error message:
...
To check whether KB3139923 is installed, go to **Windows Settings** > **Windows Update** > **Update history**. Or use PowerShell to export a list of all installed hotfixes:
